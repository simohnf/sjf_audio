//
//  sjf_interpolators.h
//
//  Created by Simon Fay on 24/08/2022.
//

#ifndef sjf_interpolators_h
#define sjf_interpolators_h

#include <JuceHeader.h>
#include <vector>
#include "sjf_audioUtilities.h"


//==============================================================================
//==============================================================================
//==============================================================================
//                          VALUE BASED INTERPOLATIONS
//==============================================================================
//==============================================================================
//==============================================================================
//==============================================================================


template <typename T>
const T linearInterpolate ( const T &mu, const T &y1, const T &y2 )
{
    return y1 + mu*(y2-y1) ;
}
//==============================================================================
template <typename T>
const T cubicInterpolate ( const T &mu, const T &y0, const T &y1, const T &y2, const T &y3 )
{
    T a0,a1,a2,a3,mu2;
    
    mu2 = mu*mu;
    a0 = y3 - y2 - y0 + y1;
    a1 = y0 - y1 - a0;
    a2 = y2 - y0;
    a3 = y1;
    
    return (a0*mu*mu2 + a1*mu2 + a2*mu + a3);
}
//==============================================================================
template <typename T>
const T fourPointInterpolatePD ( const T &mu, const T &y0, const T &y1, const T &y2, const T &y3 )
{
    auto y2minusy1 = y2-y1;
    return y1 + mu * (y2minusy1 - 0.1666667f * (1.0f - mu) * ( (y3 - y0 - 3.0f * y2minusy1) * mu + (y3 + 2.0f*y0 - 3.0f*y1) ) );
}

//==============================================================================
template <typename T>
const T fourPointFourthOrderOptimal  ( const T &mu, const T &y0, const T &y1, const T &y2, const T &y3 )
{
    //    Copied from Olli Niemitalo - Polynomial Interpolators for High-Quality Resampling of Oversampled Audio
    // Optimal 2x (4-point, 4th-order) (z-form)
    T z = mu - 0.5f;
    T even1 = y2+y1, odd1 = y2-y1;
    T even2 = y3+y0, odd2 = y3-y0;
    T c0 = even1*0.45645918406487612 + even2*0.04354173901996461;
    T c1 = odd1*0.47236675362442071 + odd2*0.17686613581136501;
    T c2 = even1*-0.253674794204558521 + even2*0.25371918651882464;
    T c3 = odd1*-0.37917091811631082 + odd2*0.11952965967158000;
    T c4 = even1*0.04252164479749607 + even2*-0.04289144034653719;
    return (((c4*z+c3)*z+c2)*z+c1)*z+c0;
    
}

//==============================================================================
template <typename T>
const T cubicInterpolateGodot ( const T &mu, const T &y0, const T &y1, const T &y2, const T &y3 )
{
    //    Copied from Olli Niemitalo - Polynomial Interpolators for High-Quality Resampling of Oversampled Audio
    T a0,a1,a2,a3,mu2;
    mu2 = mu*mu;
    
    a0 = 3 * y1 - 3 * y2 + y3 - y0;
    a1 = 2 * y0 - 5 * y1 + 4 * y2 - y3;
    a2 = y2 - y0;
    a3 = 2 * y1;
    
    return (a0 * mu * mu2 + a1 * mu2 + a2 * mu + a3) * 0.5f;
}
//==============================================================================
template <typename T>
const T cubicInterpolateHermite ( const T &mu, const T &y0, const T &y1, const T &y2, const T &y3 )
{
    T a0,a1,a2,a3;

    a0 = y1;
    a1 = 0.5f * (y2 - y0);
    a2 = y0 - (2.5f * y1) + (2 * y2) - (0.5f * y3);
    a3 = (0.5f * (y3 - y0)) + (1.5f * (y1 - y2));
    return (((((a3 * mu) + a2) * mu) + a1) * mu) + a0;
}

//==============================================================================
template <typename T>
const T cubicInterpolateHermite2 ( const T &mu, const T &y0, const T &y1, const T &y2, const T &y3 )
{
    T diff = y1 - y2;
    T a1 = y2 - y0;
    T a3 = y3 - y0 + 3.0f * diff;
    T a2 = -(2 * diff + a1 + a3);
    return 0.5f * ((a3 * mu + a2) * mu + a1) * mu + y1;
}

#endif /* sjf_interpolators_h */

