...
//        return ( m_sliceLenSamps[ voiceNumber ]*multiplier ) / hostQuarterNoteSamps ;
        ...
        multiplier = 1.0f;
        const auto vSliceLen = m_sliceLenSamps[ voiceNumber ];
        if ( vSliceLen == v0SliceLen ) { multiplier = 1; }
        else if ( vSliceLen < v0SliceLen )
        {
            while ( vSliceLen * multiplier < v0SliceLen ){ multiplier *= 2; }
            auto lastDiff = abs( v0SliceLen - vSliceLen*multiplier );
            auto halfMultiplier = multiplier * 0.5;
            auto newDiff = abs( v0SliceLen - vSliceLen*halfMultiplier );
            if ( newDiff < lastDiff ) { multiplier = halfMultiplier; }
        }
        else
        {
            while ( vSliceLen > v0SliceLen *multiplier ){ multiplier *= 2; }
            float lastDiff = abs( v0SliceLen*multiplier - vSliceLen );
            auto halfMultiplier = multiplier * 0.5f;
            auto newDiff = abs( v0SliceLen*halfMultiplier - vSliceLen );
            if ( newDiff < lastDiff ) { multiplier = halfMultiplier; }
            multiplier = 1/multiplier;
        }
        return v0Compensation * ( vSliceLen*multiplier ) / v0SliceLen;
    };
