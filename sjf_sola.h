...
//    ~sjf_sola(){}
    ...
    static inline void stretch( juce::AudioBuffer< T > &sampleToStretch, juce::AudioBuffer< T > &stretchedSample, T stretchFactor, int N = 2048, int hopSize = 256 )
    {
...
//            fade.emplace_back( static_cast< T >( i ) / static_cast< T >( L ) );
        ...
        for ( auto b = 1; b < M; b++ )
        {
            ...
            auto blockAOverlapStart = (b-1)*Sa + Ss;
            //      block B start is b*Sa
            auto blockBStart = b*Sa;
            auto timeLag = calculateCrossCorrelationTimeLag( sampleToStretch, blockAOverlapStart, blockBStart, L, norm );
            ...
            for ( auto s = 0; s < crossOverLength; s++ )
            {
                ...
                for ( auto c = 0; c < nChannels; c++ )
                {
                    sampVal = fadeOut <= 0 ? 0 : sampleToStretch.getSample( c, segAIndex ) * fadeOut;
                    sampVal += fadeIn <= 0 ? 0 : sampleToStretch.getSample( c, segBIndex ) * fadeIn;
            ...
            for ( auto s = 0; s < Ss - crossOverLength; s++ )
            {
                ...
                for ( auto c = 0; c < nChannels; c++ )
                    stretchedSample.setSample( c, solaIndex, sampleToStretch.getSample( c, segBIndex ) );
                solaIndex++;
            }
        }
        // resize samples
