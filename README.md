...
*   **Hierarchical & Dynamic GUI Generation:** Parameter groups generated via ParameterFactory produce rich structural metadata (GroupMetadata). This metadata automatically drives generic macro editors, collapsible modular sub-panels, and self-contained reorderable viewports (SequenceListView) with zero boilerplate delegation code. 
...
| **`DynamicProcessorSequence`** | Dynamic runtime DSP reordering           | Combines compile-time variadic execution with dynamic runtime item reordering, additions, and removals via ValueTree state bindings.                                                                                                                                          |
| **`GenericEditor`**            | Automatic Hierarchical GUI Generation    | Reads GroupMetadata generated during parameter construction to build complex, responsive control layouts. Automatically handles nested sub-editors, dynamic item reordering/swapping, and viewport scroll container scaling directly through ValueTree state bindings.        |
